...
def create_table2():
    ...
    def form_submit2():
        ...
        with connect('database.db') as db:
            cursor = db.cursor()
            query = """ INSERT INTO table2(name, expenses) VALUES (?, ?)"""
            cursor.execute(query, insert_inf)
            db.commit()
            refresh2()
...
create_new_table=ttk.Button(frame_change, text='Создать таблицу', command=create_table2)
